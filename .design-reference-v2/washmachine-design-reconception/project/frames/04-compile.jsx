...
function FrameCompile() {
  return (
    <Shell active="compile" crumbs={["Compile"]} pipeActive="cmp" running
      ...
      <div className="cfg">
        ...
        {/* Output target */}
        <Sec title="Output" action={<Chip kind="acc">{`{ts}-{sha8}.exe`}</Chip>}>
          <div className="card">
            <div className="row" style={{ gap: 16, alignItems: "flex-end" }}>
              <Field label="Filename pattern" hint="{ts} = build time · {sha8} = first 8 of source hash">
                <input className="input mono" defaultValue="{ts}-{sha8}.exe" style={{ width: 240 }} />
              </Field>
              <Field label="Directory" hint="resolved from AppPaths.OutputDirectory">
                <input className="input mono" defaultValue="C:\\Users\\hmza\\washmachine\\out\\" style={{ width: 320 }} />
              </Field>
              <div style={{ flex: 1 }} />
              <Field label="Save session artifacts" hint="source.cpp · build_log · UIData">
                <Toggle on />
              </Field>
            </div>
          </div>
