...
const NAV_PRIMARY = [
  ...
  { id: "compile",     name: "Compile",     icon: "play" },
];
const NAV_FOOTER = [
      ...
      <div className="rail-nav">
        {NAV_PRIMARY.map(n => (
          <div key={n.id} className={"rail-item" + (n.id === active ? " active" : "")}>
            <div className="ri-ico"><Icon name={n.icon} size={15} /></div>
            <div>{n.name}</div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="pipe">
      ...
      <div className="pipe-run">
        {running ? (
          <button className="btn" style={{ borderColor: "oklch(0.55 0.14 25 / 0.35)", color: "var(--err)" }}>
