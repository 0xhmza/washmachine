...
def xor_file(input_file, output_file, xorkey):
    ...
    while True:
        ...
        if len(some_bytes) % 4 != 0:
            ...
            some_bytes = some_bytes + b"\x00" * (number_added)
...
def write_rsrc(f, oldrva, newRva):
    ...
    rsrc_structure['Typeheader']['Entries'] = merge_two_dicts(rsrc_structure['Typeheader']["IDentries"],
                                                              rsrc_structure['Typeheader']['NameEntries'])
    for entry, value in rsrc_structure['Typeheader']["Entries"].items():
        ...
        #Now get language
        for name_id, offset in rsrc_structure[entry]["NameIDs"].items():
            ...
            for lanID, offsetDataEntry in rsrc_structure[name_id]["language"].items():
